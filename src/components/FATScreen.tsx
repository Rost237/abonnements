...
import type { FAT, UserRole } from "@/types";
...
export default function FATScreen({ userRole, fats, onFatsChange }: FATScreenProps) {
  ...
  const filtered = fats.filter(f => [f.nom, f.code, f.ville, f.quartier].some(v => v.toLowerCase().includes(search.toLowerCase())));

  const handleSave = () => {
    if (!form.nom || !form.code) return;
    if (editing) { onFatsChange(fats.map(f => f.id === editing.id ? { ...f, ...form } : f)); }
    else { onFatsChange([...fats, { ...form, id: crypto.randomUUID() }]); }
    setForm(emptyFAT); setEditing(null); setOpen(false);
  };

  const handleEdit = (fat: FAT) => { setEditing(fat); setForm({ nom: fat.nom, code: fat.code, ville: fat.ville, quartier: fat.quartier, localisation: fat.localisation, description: fat.description }); setOpen(true); };
  const handleDelete = (id: string) => onFatsChange(fats.filter(f => f.id !== id));
  const openNew = () => { setEditing(null); setForm(emptyFAT); setOpen(true); };
    ...
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center justify-between">
        ...
        {isAdmin && (
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button onClick={openNew} className="h-10 rounded-md gap-2"><Plus size={16} /> Nouveau FAT</Button></DialogTrigger>
            <DialogContent className="sm:max-w-md">
              <DialogHeader><DialogTitle>{editing ? "Modifier le FAT" : "Nouveau FAT"}</DialogTitle></DialogHeader>
              <div className="space-y-3 pt-2">
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5"><Label className="text-foreground">Nom *</Label><Input value={form.nom} onChange={e => setForm(f => ({ ...f, nom: e.target.value }))} placeholder="FAT Akwa" className="h-10 rounded-md" /></div>
                  <div className="space-y-1.5"><Label className="text-foreground">Code *</Label><Input value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value }))} placeholder="FAT-001" className="h-10 rounded-md font-mono" /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
      ...
      <div className="bg-card rounded-lg shadow-card overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              <TableRow><TableCell colSpan={isAdmin ? 5 : 4} className="text-center text-muted-foreground py-8">Aucun FAT</TableCell></TableRow>
            ) : filtered.map(f => (
              <TableRow key={f.id}>
                ...
                {isAdmin && <TableCell><div className="flex gap-1"><button onClick={() => handleEdit(f)} className="p-1.5 rounded-md hover:bg-accent text-muted-foreground hover:text-foreground"><Pencil size={14} /></button><button onClick={() => handleDelete(f.id)} className="p-1.5 rounded-md hover:bg-destructive/10 text-muted-foreground hover:text-destructive"><Trash2 size={14} /></button></div></TableCell>}
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </div>
      <p className="text-xs text-muted-foreground">{fats.length} FAT(s)</p>
    </div>
  );
}
